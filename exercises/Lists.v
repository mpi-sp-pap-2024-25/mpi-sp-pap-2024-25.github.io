...
statement for being able to prove it by induction.
Intuitively, the reason for this is that a more general statement
also yields a more general (and hence stronger) inductive hypothesis.
For this reason, when being stuck in a proof, it can make sense to
step back and see whether it is possible to prove a stronger statement. *)
...
(** A first attempt may be to prove exactly the statement that
we are missing at this point.
However, we will fail to to so since the inductive hypothesis resulting
from it is not general enough to conclude the proof. *)
...
(** We can slightly strengthen the lemma to not only work on
reversed lists but general lists. *)
...
Proof.
  ...
  - (* l = m:: l' *)
    ...
    reflexivity.
Qed.
(** This generalized lemma  would be sufficient to conclude
our original proof.
Still, we could prove an even more general lemma about
the length of appended lists.  *)
...
(** To illustrate how induction on more complex datatypes works,
we will introduce the datatype [btree] of labeled trees that are
at most binary:
This means that a each node in tree is labeled with a natural number
and has at most two children.
...
(** We define a function [size] that counts the number of nodes
in the tree. *)
...
(** We introduce a helper lemma for doing a specific arithmetic
transformation that we will use in the theorem that we want to prove next.
It only requires a combination of associativity and commutativity of
addition but since it involves several of such steps it simplifies the main
proof to have it as a seperate lemma. *)
...
(** We can state the following theorem on the sum of labels of a tree
with incremented labels.
The proof proceeds by induction on the tree [t].
Consequently, we will need to to prove three cases:
- The base case [t = leaf n]
- The case [t = unary n t'] for a unary tree [t]
with inductive hypothesis [IHt'] for the child tree [t']
- The case [t = binary n t1' t2'] for a binary tree [t]
with inductive hyptheses [IHt1'] for child tree [t1']
